...
c.execute("SELECT * FROM source_transactions LIMIT 1000")
items = c.fetchall()
...
for row_item in items:
    ...
    composite_key = str(store_key).zfill(3) + str(date_key).zfill(3) + str(product_key).zfill(4)
...
text_file = open("deliverable6.csv", "w")

for row in final_data:
    text_file.write("%s,%s,%s,%s,%s,%s,%s,%s\n" % (final_data[row]['composite_key'], final_data[row]['date_key'], final_data[row]['product_key'], final_data[row]['store_key'], final_data[row]['quantity_sold'], final_data[row]['total_cost_to_store'], final_data[row]['total_dollar_sales'], final_data[row]['gross_profit']))
